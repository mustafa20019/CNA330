...
def create_tables(cursor, table):
    ...
    cursor.execute('''CREATE TABLE IF NOT EXISTS New_Job (id INT PRIMARY KEY auto_increment, Type varchar(10), Title varchar(100), Description text,
    Job_id varchar(33), Created_at DATE, Company varchar(100), Location varchar(50), How_to_apply varchar(100)); ''')
    cursor.execute('''INSERT INTO `new_job` (`id`, `Type`, `Title`, `Description`, `Job_id`, `Created_at`, `Company`, `Location`, `How_to_apply`) 
                    VALUES ('1', 'Full_time', 'JavaScript Developer', 'looking for exceptionally motivated candidates', '5', '2019-10-03', 'Dell', 'Seattle', 'www.dell.com');''')
...
def check_if_job_exists(cursor, jobdetails):
    ## Add your code here
    cursor.execute('''SELECT * FROM 'new_job''')
    query = "SELECT"
    return query_sql(cursor, query)

def delete_job(cursor, jobdetails):
    ## Add your code here
    cursor.execute('''DELETE FROM new_job''')
    query = "UPDATE"
    return query_sql(cursor, query)
...
def jobhunt(cursor,arg_dict): # Important, rest are supporting functions
